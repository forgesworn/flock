...
class OkHttpRelayPublisher(private val timeoutMs: Long = 10_000) : RelayPublisher {
    ...
    override fun publish(relayUrls: List<String>, eventJson: String): Int {
        ...
        val sockets = relayUrls.mapNotNull { url ->
            try {
                val alreadyOK = AtomicBoolean(false)
                client.newWebSocket(Request.Builder().url(url).build(), object : WebSocketListener() {
                    ...
                    override fun onMessage(webSocket: WebSocket, text: String) {
                        try {
                            ...
                            if (msg.getString(0) == "OK" && msg.getString(1) == eventId && !alreadyOK.getAndSet(true)) {
                ...
                // Malformed URL or other socket creation failure; skip this relay.
                done.countDown()
                null
            }
        }
